...
- **Duplicate trade buttons in settlement UI**: Settlement overlay renders two "Trade" buttons — one for the trade panel toggle and one for the settlement action. These collide when both appear. Fix: separate the trade panel action from the settlement action button.
