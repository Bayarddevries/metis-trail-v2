// ═══════════════════════════════════════════════════════════
// GAME ENGINE (embedded — same logic as src/engine.js)
// ═══════════════════════════════════════════════════════════
...
const ITEMS = [
  ...
  { name: 'Medicine Pouch',   wt: 2,  count: 3,  type: 'medical', desc: 'Herbal remedies and bandages.' },
...
function createGame(seed) {
  ...
  const S = {
    ...
    food: 60, wear: 0, squeal: 0, node: 0, segmentDay: 0, travelDaysWithoutRest: 0,
  ...
  function settlementAction(action) {
    ...
    if (action === 'rest') {
      ...
      S.day++;
      advanceMonth();
      say("You rest for the day. The crew recovers. You consume 2 food.");
    }
    if (action === 'repair') {
      const shagItem = cart.find(i => i.name === 'Shaganappi');
      const toolItem = cart.find(i => i.name === 'Tool Kit');
      if (S.wear > 0 && shagItem && shagItem.count > 0 && toolItem && toolItem.count > 0) {
        shagItem.count--;
        toolItem.count--;
        S.wear = Math.max(0, S.wear - 2);
        say(`You repair the cart using shaganappi strips and tool kit. Wear reduced to ${S.wear}.`);
      } else if (S.wear > 0) {
...
// ── Map init (deferred until after intro) ───────────────────────
let map = null, trailLine = null, markers = null, playerMarker = null;

// Red River Cart SVG icon
// Cart marker using the Red River cart illustration
const cartIcon = L.divIcon({
  className: 'cart-marker',
  html: '<img src="art/cart_marker_48px.png" width="48" height="28" style="display:block;filter:drop-shadow(1px 2px 3px rgba(0,0,0,0.4));" alt="Cart" />',
  iconSize: [48, 28],
  iconAnchor: [24, 28]
});
...
function initMap() {
  if (map) return;
  map = L.map('map', { zoomControl: false, attributionControl: false }).setView([51.5, -102], 5);
  L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}.png', { maxZoom: 8, minZoom: 4 }).addTo(map);
  trailLine = L.polyline(NODES.map(n => [n.lat, n.lon]), { color: '#8B7355', weight: 3, opacity: 0.6, dashArray: '8,4' }).addTo(map);
  markers = NODES.map((n, i) => {
    const m = L.circleMarker([n.lat, n.lon], {
      radius: 4, fillColor: '#B8A478', color: '#8B7355', weight: 1.5, fillOpacity: 0.7
    }).addTo(map);
    m.bindTooltip(n.name, { permanent: false, direction: 'top' });
    return m;
  });
  // Player cart marker
  playerMarker = L.marker([NODES[0].lat, NODES[0].lon], { icon: cartIcon, zIndexOffset: 1000 }).addTo(map);
}
...
function updateMap() {
  ...
  // Pan camera to current node (animated)
  map.panTo([node.lat, node.lon], { animate: true, duration: 0.5 });
  // Move cart marker to current node
  playerMarker.setLatLng([node.lat, node.lon]);
  // Dim past nodes, highlight current
  markers.forEach((m, i) => {
    if (i === s.node) { m.setRadius(6); m.setStyle({ fillColor: '#8B2500', fillOpacity: 1, stroke: '#1A1410', weight: 2 }); }
    else if (i < s.node) { m.setRadius(4); m.setStyle({ fillColor: '#9A8A7A', fillOpacity: 0.4, stroke: '#B8A478', weight: 1 }); }
    else { m.setRadius(4); m.setStyle({ fillColor: '#B8A478', fillOpacity: 0.7, stroke: '#8B7355', weight: 1.5 }); }
  });
}
...
function updateStatus() {
  ...
  document.getElementById('s-day').textContent = 'Day ' + s.day;
  document.getElementById('s-month').textContent = s.month + ' ' + (((s.day - 1) % 30) + 1);
  document.getElementById('s-season').textContent = (s.season || 'summer').replace('_', ' ');

  const segEl = document.getElementById('s-segment');
  if (s.pendingSettlement) {
    segEl.textContent = s.location + ' — you have arrived';
  } else if (next) {
    segEl.textContent = `${s.location} → ${next.name} (${s.segmentDay}/${s.segmentDist} days)`;
  } else {
    segEl.textContent = s.location;
  }

  const foodEl = document.getElementById('s-food');
  foodEl.textContent = s.food;
  foodEl.className = 'stat-value' + (s.food <= 3 ? ' food-low' : '');

  const wearEl = document.getElementById('s-wear');
  wearEl.textContent = s.wear;
  wearEl.className = 'stat-value' + (s.wear >= 3 ? ' wear-high' : '');

  document.getElementById('s-crew').textContent = s.crew.charAt(0).toUpperCase() + s.crew.slice(1);
}

function showNarrative() {
  const n = game.getNarrative();
  document.getElementById('narrative').innerHTML = n.map(t => `<div class="scene-text">${t}</div>`).join('');
...
function presentEvent() {
  ...
  if (!ev) return;
  const _eEmoji = { rough_ground:'🪨', prairie_fire:'🔥', buffalo_herd:'🦬', game_sighted:'🦌', abandoned_camp:'🛺', snake_in_cart:'🐍', good_grazing:'🌿', trail_marker:'🪧', prairie_wind:'💨', thunderhead:'⛈️', meadowlark:'🐦', dried_waterhole:'🏜️', ox_injury:'🦶', other_travelers:'🛺', spring_flooding:'🌊', fishing_spot:'🎣', willow_shade:'🌳', mud_hole:'🟤', riverbank_collapse:'💥', mosquito_plague:'🦟', broken_ferry:'⛴️', friendly_métis:'⛪', flash_flood:'🌧️', fallen_tree:'🪵', berries:'🫐', lost_trail:'🧭', bear_encounter:'🐻', broken_wheel:'🔧', deer_crossing:'🦌', firewood_opportunity:'🪵', old_cabin:'🏠', steep_descent:'⛰️', eagle:'🦅', rockslide:'🪨', wind_exposure:'🌬️', lost_in_fog:'🌫️', major_crossing:'🌊' };
    document.getElementById('event-art').textContent = (_eEmoji[ev.id] || '❓') + '  ' + ev.id.replace(/_/g, ' ');
  document.getElementById('event-text').innerHTML = '<p>' + ev.text.replace(/\n\n/g, '</p><p>') + '</p>';
  ...
  choices.innerHTML = '';
  console.log('presentEvent', ev.id, 'choices:', ev.choices?.length);
  if (!ev.choices || !Array.isArray(ev.choices)) {
    console.error('presentEvent: ev.choices is not an array!', ev.id, ev.choices);
  }
  ev.choices.forEach((ch, i) => {
    ...
    btn.onclick = () => {
      ...
      // Hide choices, show dice roll
      choices.style.display = 'none';
      const result = game.chooseEventChoice(i);
      const inner = Array.isArray(result) ? (result[0]?.result || null) : result;
      showDiceRoll(inner, ch, () => {
        showNarrative();
        updateStatus();
        updateMap();
      });
  ...
  // Continue fallback
  const _choices = document.getElementById('event-choices');
  if (_choices && !_choices.querySelector('.continue-fallback')) {
    const _cntBtn = document.createElement('button');
    _cntBtn.className = 'choice-btn continue-fallback';
    _cntBtn.textContent = 'Continue';
    Object.assign(_cntBtn.style, { display:'inline-block', visibility:'visible', opacity:'1', minWidth:'120px', minHeight:'44px', background:'#8b0000', color:'#fff', fontWeight:'bold', marginTop:'8px' });
    _cntBtn.onclick = function() {
      if (_cntBtn.dataset.clicked) return;
      _cntBtn.dataset.clicked = '1';
      console.warn('Continue fallback used for', ev.id);
      game.pendingEvent = null;
      document.getElementById('event-overlay').classList.remove('active');
    };
    _choices.appendChild(_cntBtn);
  }
}

function showDiceRoll(result, choice, onComplete) {
  ...
  // Guard: if result is null/undefined (e.g. double-click), bail out
  if (!result) {
    console.warn('showDiceRoll: no result, skipping');
  ...
  if (result.roll !== null) {
    ...
    const interval = setInterval(() => {
      ...
      dieFace.textContent = fakeRoll;
      dieFace.style.transform = `rotate(${rolls * 45}deg) scale(${1 + rolls * 0.05})`;
      rolls++;
      if (rolls >= maxRolls) {
        clearInterval(interval);
        dieFace.textContent = result.roll;
        dieFace.style.transform = 'rotate(0deg) scale(1.3)';
        // Show details after a pause
        setTimeout(() => {
          let detailText = `<strong>d${result.roll}`;
          if (result.total !== null) detailText += ` + ${result.total - result.roll} = ${result.total}`;
          if (result.dc !== null) detailText += ` vs DC ${result.dc}`;
          detailText += result.success ? '</strong> — ✅ <strong>SUCCESS</strong>' : '</strong> — ❌ <strong>FAILURE</strong>';
          details.innerHTML = detailText;
          details.className = 'dice-details ' + (result.success ? 'dice-success' : 'dice-fail');

          // Show result text
          setTimeout(() => {
            diceEl.style.display = 'none';
            resultEl.style.display = 'block';
            resultEl.innerHTML = '<p>' + result.text + '</p>';
            if (result.effects.length > 0) {
              resultEl.innerHTML += '<div class="effect-list">' + result.effects.map(e => `<span class="effect-tag">${e}</span>`).join('') + '</div>';
            }
    ...
    // No dice roll (always outcome)
    dieFace.textContent = '—';
    details.innerHTML = '<strong>No roll required</strong>';
...
function presentSettlement() {
  ...
  if (!game.getState().pendingSettlement) return;

  const _sEmoji = { hbc:'🏰', mission:'⛪', metis:'🛖', trading:'🏪', nwmp:'⭐', river:'⛵' }; document.getElementById('settlement-art').textContent = (_sEmoji[node.type] || '🏳') + '  ' + node.name;
  document.getElementById('settlement-name').textContent = node.name;
  document.getElementById('settlement-desc').textContent = node.desc;

  const actions = game.getAvailableActions();
  const container = document.getElementById('settlement-actions');
  container.innerHTML = '';

  const labels = {
    trade: '🛒 Trade', repair: '🔧 Repair Cart', rest: '🏕 Rest', heal: '💊 Seek Healing', continue: '🚩 Continue West'
  };
...
function showCart() {
  ...
  const wt = game.getTotalWeight();
  const pct = Math.min((wt / 450) * 100, 100);
  fill.style.width = pct + '%';
  fill.classList.toggle('over', wt > 450);
  txt.textContent = wt + ' / 450 kg' + (wt > 450 ? ' — OVERLOADED!' : '');
  list.innerHTML = '';
  cart.forEach(it => {
    if (it.count <= 0) return;
    const d = document.createElement('div');
    d.className = 'inv-item';
    d.innerHTML = `<span>${it.name} ×${it.count}</span><span>${it.wt * it.count} kg</span>`;
    list.appendChild(d);
  });
...
function showCrew() {
  const s = game.stateSummary();
  const desc = s.crew === 'rested' ? 'Your crew is rested and ready. The ox is strong. You feel confident.'
    : s.crew === 'tired' ? 'Your crew is tired. The ox is slowing. You need rest soon.'
    : 'Your crew is exhausted. The ox can barely walk. You must rest immediately or risk collapse.';
  document.getElementById('crew-status').innerHTML = `<p>${desc}</p>`;
  document.getElementById('crew-modifier').textContent = `Modifier to checks: ${s.crew === 'rested' ? '+1' : s.crew === 'tired' ? '+0' : '-2'}`;
...
function showEndScreen() {
  ...
  const tgCount = tradeGoods.reduce((sum, i) => sum + i.count, 0);

  document.getElementById('end-title').textContent = s.won ? 'You Made It!' : 'Journey Over';

  const narrative = game.getNarrative();
  const lastNarrative = narrative[narrative.length - 1] || '';
  document.getElementById('end-narrative').innerHTML = '<p>' + lastNarrative.replace(/\n\n/g, '</p><p>') + '</p>';

  document.getElementById('end-stats').innerHTML = `
    <div class="stat-row"><span class="label">Days on trail</span><span>${s.day}</span></div>
    <div class="stat-row"><span class="label">Food remaining</span><span>${s.food}</span></div>
    <div class="stat-row"><span class="label">Cart wear</span><span>${s.wear} / 4</span></div>
    <div class="stat-row"><span class="label">Trade goods delivered</span><span>${tgCount}</span></div>
    <div class="stat-row score-row"><span class="label">Score</span><span>${s.score}</span></div>
  `;

  document.getElementById('end-overlay').classList.add('active');
}

// ── Check which overlay to show ─────────────────────────────────
function checkOverlays() {
  if (game.isOver()) { showEndScreen(); return; }
  if (game.hasPendingEvent()) { presentEvent(); return; }
  if (game.getState().pendingSettlement) { presentSettlement(); return; }
}
...
function doAction(action) {
  ...
  if (action === 'travel') {
    if (game.getAvailableActions().type !== 'travel') return;
    game.travelOneDay();
  } else if (action === 'camp') {
    if (game.getAvailableActions().type !== 'travel') return;
    game.makeCamp();
...
// ── Button handlers ─────────────────────────────────────────────
document.getElementById('btn-travel').onclick = () => doAction('travel');
document.getElementById('btn-camp').onclick = () => doAction('camp');
document.getElementById('btn-cart').onclick = () => showCart();
document.getElementById('btn-crew').onclick = () => showCrew();

document.getElementById('cart-close-btn').onclick = () => { document.getElementById('cart-overlay').classList.remove('active'); };
document.getElementById('crew-close-btn').onclick = () => { document.getElementById('crew-overlay').classList.remove('active'); };
document.getElementById('settlement-close').onclick = () => { document.getElementById('settlement-overlay').classList.remove('active'); };
...
// ── Init ────────────────────────────────────────────────────────
// Game starts after intro screen — don't call updateStatus() yet
